...
namespace SeeAPsychologist.WorldState
{
    ...
    /// - 平时隐藏提示文字（如 "put F"）
    /// - 玩家进入触发范围后显示提示
    /// - 玩家在范围内按 F 时，调用 BedWorldSwitchInteractable.Interact()，由其按解离度决定是否切换世界
    ///
    /// 说明：
    /// - 这里的 Update 只做“按键触发事件检测”，不做任何随时间自动增减数值的玩法逻辑（符合项目硬性规则）。
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class BedFInteractController : MonoBehaviour
    {
        ...
        [Tooltip("是否在交互后立即隐藏提示（不管是否切换）。")]
        [SerializeField] private bool hidePromptAfterInteract = false;

        private BedWorldSwitchInteractable _interactable;
        private int _inRangeCount;

        private void Awake()
        {
            _interactable = GetComponent<BedWorldSwitchInteractable>();
            if (_interactable == null)
            {
                Debug.LogWarning("[A3][WorldState] BedFInteractController requires BedWorldSwitchInteractable on the same GameObject.", this);
            }
